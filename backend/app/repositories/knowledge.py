...
def replace_chunks(paper_id: str, chunks: list[dict[str, Any]]) -> None:
    ...
    with get_connection() as conn:
        # Embeddings are keyed by chunk index rather than a chunk row ID. Clear
        # them in the same transaction so a failed or unavailable re-embedding
        # step cannot leave vectors from the previous chunk set attached to new
        # content.
...
def get_paper_embeddings(paper_id: str) -> list[dict[str, Any]]:
    with get_connection() as conn:
        rows = conn.execute(
            """
